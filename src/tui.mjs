...
${c.bold("Lệnh:")}
  /help            xem trợ giúp
  /model <id>      đổi model (vd: /model qwen3.6-plus)
  /models          liệt kê model gói Go
  /yolo            bật tự duyệt mọi thao tác (write/edit/bash)
  /safe            tắt tự duyệt (mặc định)
...
export async function runTui({ model = DEFAULT_MODEL } = {}) {
  ...
  const agent = createAgent({
    ...
    onEvent(ev) {
      ...
      switch (ev.type) {
        case "thinking": {
          const fr = ["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"]; let i = 0;
          spin = setInterval(() => process.stdout.write("\r" + c.magenta(fr[i++ % fr.length]) + c.dim(` ${ev.model} đang nghĩ…  `)), 80);
          break;
        }
        case "assistant":
        ...
        case "tool_denied":
          process.stdout.write(c.red("    ✗ đã từ chối\n"));
          break;
        case "usage":
          ...
          break;
        case "max_steps":
          process.stdout.write(c.yellow("  ⚠ đạt giới hạn số bước\n"));
          break;
      }
    },
    async approve(name, args, preview) {
      ...
      stopSpin();
      const a = (await ask(c.yellow(`  ? chạy ${c.bold(name)}(${String(preview).slice(0, 60)})? [y/N/a=luôn] `))).trim().toLowerCase();
  ...
  for (;;) {
    ...
    if (input.startsWith("/")) {
      ...
      else if (cmd === "yolo") { autoApprove = true; process.stdout.write(c.yellow("  YOLO: tự duyệt mọi thao tác\n")); }
      else if (cmd === "safe") { autoApprove = false; process.stdout.write(c.dim("  SAFE: hỏi trước khi write/edit/bash\n")); }
      else if (cmd === "skills") process.stdout.write([...agent.skills.values()].map((s) => `  ${c.bold(s.name)} — ${c.dim(s.description)}`).join("\n") + "\n" || "  (không có skill)\n");
      else if (cmd === "clear") { agent.reset(); process.stdout.write(c.dim("  đã xoá lịch sử\n")); }
      else process.stdout.write(c.red(`  lệnh lạ: /${cmd}\n`));
      continue;
    }
  ...
  rl.close();
  process.stdout.write(c.dim("tạm biệt 👋\n"));
}
